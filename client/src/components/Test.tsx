...
const Scorecard: FC<Props> = ({ frameList }) => {
  ...
  function calculateMaxScore(fList: number[][]): number {
    ...
    for (let i = 0; i < 9; i++) {
      ...
      } else if (fList[i][0] !== 10) {
        fList[i][1] = 10 - fList[i][0];
        fList[i][1] = 69;
      }
    }
    // 10th frame
    if (!fList[9]) {
      ...
      if (fList[9].length === 1) {
        if (fList[9][0] === 10) {
          ...
          fList[9][1] = 10 - fList[9][0];
          fList[9][2] = 10;
        }
      }
      if (fList[9].length === 2) {
        if (fList[9][1] === 10 || fList[9][1] + fList[9][0] === 10) {
          fList[9][2] = 10;
        } else {
          fList[9][2] = 10 - fList[9][1];
        }
      }
    }

    // maxScore = calcFrameScores(convertFrameList([...fList]))[9];
  ...
  return (
    <>
      <CurrentGameStats frameList={frameList} frameScores={frameScores} maxScore={calculateMaxScore([...frameList])} />
